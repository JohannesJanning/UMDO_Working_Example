...
from bat_run_qbit_MCS import RobustOptimizer, SizingResult, inner_solve_for_Wtotal, sample_battery_density
...
def main():
    ...
    x_det = [30.82883175888127, 0.22546383016044785, 1.2999999999999603, 0.20047005595979095]
    ...
    if stats is not None:
        ...
        if use_joblib:
            ...
            per_results = Parallel(n_jobs=uq.n_jobs, prefer='processes')(
                delayed(inner_solve_for_Wtotal)(rho, uq.payload_kg, uq.range_m, uq.n_c, design_vars=tuple(x_det))
                for rho in samples
            )
        else:
            for rho in samples:
                r = inner_solve_for_Wtotal(rho, uq.payload_kg, uq.range_m, uq.n_c, design_vars=tuple(x_det))
    ...
    rho_samples = np.array([p.get('BATTERY_DENSITY', np.nan) for p in valid_results if isinstance(p, dict)])
    if len(rho_samples) > 0:
        print(f'\n--- Battery Specific Energy Samples ---')
        print(f'  Mean: {np.mean(rho_samples):.1f} Wh/kg')
        print(f'  Std:  {np.std(rho_samples):.1f} Wh/kg')
        print(f'  Median: {np.median(rho_samples):.1f} Wh/kg')
        print(f'  5th percentile: {np.percentile(rho_samples, 5):.1f} Wh/kg')
        print(f'  95th percentile: {np.percentile(rho_samples, 95):.1f} Wh/kg')
    ...
    # Deterministic evaluation at median battery density (for comparison marker)
    # Note: For lognormal, median = exp(μ)
    median_rho = uq.median_rho
    det_res = inner_solve_for_Wtotal(median_rho, uq.payload_kg, uq.range_m, uq.n_c, design_vars=tuple(x_det))
    det_W = None
    det_cl = None
    ...
    ax1.set_title(f'MTOM distribution (UQ)\nBattery ρ: Lognormal(median={MEDIAN_RHO} Wh/kg, σ={SIGMA_LN})')
    ax1.legend()
    ...
    # --- Add battery density distribution plot ---
    if len(rho_samples) > 0:
        fig5, ax5 = plt.subplots(figsize=(6,4))
        ax5.hist(rho_samples, bins=50, color='#4c72b0', edgecolor='k', alpha=0.8, density=True)
        
        # Add theoretical lognormal PDF
        from scipy.stats import lognorm
        xs_rho = np.linspace(100, 500, 300)
        theoretical_pdf = lognorm.pdf(xs_rho, s=SIGMA_LN, scale=MEDIAN_RHO)
        ax5.plot(xs_rho, theoretical_pdf, 'r-', lw=2, label='Theoretical Lognormal')
        
        ax5.axvline(MEDIAN_RHO, color='green', linestyle='--', label=f'Median: {MEDIAN_RHO} Wh/kg')
        ax5.axvline(np.percentile(rho_samples, 5), color='orange', linestyle=':', label=f'5th: {np.percentile(rho_samples, 5):.0f} Wh/kg')
        ax5.axvline(np.percentile(rho_samples, 95), color='orange', linestyle=':', label=f'95th: {np.percentile(rho_samples, 95):.0f} Wh/kg')
        
        ax5.set_xlabel('Battery Specific Energy (Wh/kg)')
        ax5.set_ylabel('Density')
        ax5.set_title(f'Battery Density Samples (n={len(rho_samples)})\nLognormal(μ=5.46, σ=0.28)')
        ax5.legend()
        
        p5 = out_dir / 'uq_battery_density.png'
        fig5.tight_layout()
        fig5.savefig(p5, dpi=150)
        plt.close(fig5)
        print(f'Saved Battery Density distribution to {p5}')
