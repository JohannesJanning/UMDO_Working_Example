...
class RobustOptimizer:
    ...
    def objective(self, x: Sequence[float]) -> float:
        ...
        # Build a compact progress line (like sensitivity_screening.py) with ETA.
        # Use a short ASCII progress bar and overwrite the same terminal line.
        bar_len = 20
        try:
            hrs = int(eta_s // 3600)
        ...
        if stats is None:
            # failed inner solves — heavy penalty; print compact failure line
            line = (f"{bar} [{self._obj_calls}/{self.maxiter}] ETA {eta_str} "
                    f"→ V={V_inf:.3f}, r={r:.4f}, J={J:.3f}, S_w={S_w:.4f}  FAILED\n")
        ...
        line = (f"{bar} [{self._obj_calls}/{self.maxiter}] ETA {eta_str} "
                f"→ V={V_inf:.3f}, r={r:.4f}, J={J:.3f}, S_w={S_w:.4f} "
                f"meanW={meanW:.1f}N stdW={stdW:.1f}N U={U:.1f}")
    ...
    def run(self, x0: Sequence[float] | None = None, method: str = 'SLSQP'):
        ...
        def constr_cruise_CL(x):
            ...
            if stats is None:
                return -1e6
            # enforce an additional margin below the CL_MAX
