...
def setup_and_init_uqpce() -> dict:
    ...
    # For T = shift + ln(X) where X ~ LogNormal(μ, σ)
    # E[T] = shift + exp(μ + σ²/2) = target_mean
    # Var[T] = exp(2μ + σ²)(exp(σ²) - 1) = target_std²
    ...
    # IMPORTANT: UQPCE expects 'mean' and 'stdev' (not 'mu' and 'sigma')
    config = {
        "Variable 0": {
            "name": "z_hover",  # Standard normal variable
            "distribution": "normal",
            "mean": 0.0,
            "stdev": 1.0,
            "type": "aleatory",
    ...
    with warnings.catch_warnings():
        ...
        X = pce.sample()  # This gives N(0,1) samples
        np.savetxt(MATRIX_FILE, X)

    d = interface.initialize_dict(YAML_INPUT, MATRIX_FILE)
    
    # Store transform parameters for use in QBiTUQComp
...
class QBiTUQComp(om.ExplicitComponent):
    """
    Evaluates QBiT with lognormal t_hover via transform trick.
    UQPCE provides N(0,1) samples, component transforms to lognormal.
    """

    def initialize(self):
        self.options.declare("resp_cnt", types=int)
        self.options.declare("z_samples", types=np.ndarray, desc="N(0,1) samples from PCE")
    ...
    def setup(self):
        ...
        # Inner problem setup (same as before)
...
# ---------------------------------------------------------------------------
def run():
    print("Setting up UQPCE with lognormal transform...")
    d = setup_and_init_uqpce()
    n_quad = d["resp_cnt"]
    z_samples = d["run_matrix"][:, 0]  # N(0,1) samples
    lognormal_params = d["lognormal_params"]
    
    print(f"  PCE uses Normal(0,1) → Hermite polynomials")
    ...
    # QBiT evaluator with lognormal transform
    comp = QBiTUQComp(
        resp_cnt=n_quad,
        z_samples=z_samples,
        payload_kg=3.0,
    ...
    # ---- PCE validation at initial design ----------------
    ...
    # ---- Run optimisation (ONLY ONCE!) ---------------------------------
    print("\nRunning UQPCE robust optimisation with exact lognormal...")
    prob.run_driver()  # ← REMOVED the duplicate call
